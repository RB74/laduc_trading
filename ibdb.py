...
from sqlalchemy import create_engine, select, func, and_, Column, String, Integer, Float, DateTime, ForeignKey
...
class IbOrder(Base):
    ...
    @hybrid_method
    def get_ib_contract(self, session, trade_id):
        db_contract = session.query(IbContract).filter(
